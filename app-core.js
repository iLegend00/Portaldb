...
async function loadData(){
  const sets = await Promise.all(collections.map(async name=>{
    try{
      const res = await fetch(`data/${name}.json?v=20260909-chapter3-1`);
      if(!res.ok) return [];
...
function renderEquipmentArtwork(entry){
  const artwork=entry.artwork||entry.image||entry.icon;
  return artwork?`<div class="equipment-artwork"><img src="${escapeHtml(artwork)}" alt="${escapeHtml(entry._displayName)}"></div>`:"";
}
