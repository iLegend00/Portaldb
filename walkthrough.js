(()=>{
  ...
  function renderStoredNotes(part){
    ...
    if(!stored.length)return '<aside class="guide-callout recommended"><strong>PortalDB Guide Notes</strong>More progression and optimization guidance is coming soon.</aside>';
  ...
  function renderPart(part,chapter){
    ...
    return `<article class="guide-part" id="${partId(part)}" data-section-name="${esc(part.name)}"><header class="part-heading"><span class="part-number">${String(part.part).padStart(2,'0')}</span><div><small>Part ${part.part}</small><h3>${esc(part.name)}</h3></div></header><p class="part-context">${esc(part.description||'Additional progression context for this step has not been documented yet.')}</p><div class="part-details"><div><section class="guide-block"><h4>What to do</h4><ul class="objective-list">${(part.objectives||[]).map(item=>`<li>${esc(item)}</li>`).join('')}</ul></section><section class="guide-block"><h4>Rewards</h4><div class="reward-list">${(part.rewards||[]).map(reward=>`<span>${rewardText(reward)}</span>`).join('')}</div></section></div><div><section class="guide-block"><h4>Key locations</h4><div class="fact-list">${locations.length?locations.map(name=>`<span>${esc(name)}</span>`).join(''):'<span>Not specifically documented</span>'}</div></section><section class="guide-block"><h4>Important NPCs</h4><div class="fact-list">${names.length?names.map(name=>{const npc=npcByName(name);return `<span>${esc(name)}${npc?.profession?` · ${esc(npc.profession)}`:''}</span>`}).join(''):'<span>None specifically documented</span>'}</div></section>${renderStoredNotes(part)}</div></div></article>`;
  }
  function renderOverview(chapter){
    ...
    return `<div class="chapter-overview"><div class="overview-block"><small>Chapter status</small><strong>${chapter.parts.length} documented parts</strong></div>${flow.length?`<div class="overview-block"><small>Primary route</small><p>${flow.map(esc).join(' → ')}</p></div>`:''}<div class="overview-block"><small>Key NPCs</small><p>${names.length?names.map(esc).join(' · '):'No chapter-level NPC list documented'}</p></div>${unlocks.length?`<div class="overview-block"><small>Progression unlocks</small><p>${unlocks.map(esc).join(' · ')}</p></div>`:''}</div>`;
  }
  ...
  function renderFuture(){return `<section class="future-chapters" id="future-chapters"><span class="chapter-kicker">Future progression</span><h2>Chapters 3–6 are still in progress.</h2><p>These sections will be added as the walkthrough expands. PortalDB is not filling the gaps with inferred routes or recommendations.</p><div class="future-chapter-list">${futureNames().map(name=>`<span>${esc(name)} · Guide in progress</span>`).join('')}</div></section>`}
  function buildIndex(){
    ...
    const future=futureNames().map(name=>`<section class="index-chapter"><span><span>${esc(name)}</span><em>Guide in progress</em></span><div class="index-parts"><span>Walkthrough content coming soon</span></div></section>`).join('');
