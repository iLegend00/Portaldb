...
Equipment uses one consolidated Stats region. Guaranteed equipment attributes are presented together without exposing the internal Primary versus Fixed taxonomy; the structured data retains that distinction. Guaranteed Stats use aligned Stat / Range-or-Value columns, while Possible Modifiers use aligned Stat / Range / Chance columns. Each semantic table owns shared, bounded column geometry so rows scan consistently without stretching values across the dialog. Roll Quality sits directly beneath the variable stats it explains. Acquisition and recipe relationships favor direct navigation to exact PortalDB records.
